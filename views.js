...
var GUI = (function() {
  ...
  var CreateTaskView = Backbone.View.extend({
    ...
    render: function() {
      ...
      var submit = '<button id="add-task">Tweet!</button>';
      var cancel = '<button id="cancel-task">Cancel</button>';
      this.$el.html(header + error + submit + cancel);
    },
    events : {
  ...
  var RecentPostViews = Backbone.View.extend({
    id: 'recent-post-views',
    render: function() {
  ...
  var UserView = Backbone.View.extend({
    id: 'user-view',
    render: function() {
      var username = '<h2 class="logout" id="username">' + 'Welcome ' + this.model.get('username') + '!' + '</h2>';
      var logout = '<button class="logout" id="logout">Logout</button>';
      //viewing the Twitter feed
      var myFeed = new RecentPostViews({ collection: app.tasks });
      var usersTask = new DashboardView({ collection: app.tasks });
      RecentPostViews.render();
      usersTasks.render();
  ...
  var LoginView = Backbone.View.extend({ // was users view
    id: 'login-view',
    render: function() {
      console.log('hello from LoginView!');
      var error = '<p class="error"></p>';
      var loginHeader = '<h2 class="text">Log In</h2>';
      var userHeader = '<h4 class="text">Username</h4>';
      var username = '<input type="text" id="username-input">';
      var passwordHeader = '<h4 class="text">Password</h4>';
      var passwordInput = '<input type="text" id="password-input">';
      var login = '<button id="login">LogIn</button>';
      ...
      users += '</select>';
      return this.$el.html(loginHeader + error + userHeader + username + submit + selectHeader + users);
    },
    initialize : function () {
    ...
    events : {
      'click #login': 'login',
      'change #login-page': 'dashboard-view'
    },
    addUser : function () {
  ...
  function GUI(users,tasks,el) {
    ...
    var recentPostView = new RecentPostViews();
    recentPostView.render();
    //$('#app').append(recentPostView.$el);
  }
